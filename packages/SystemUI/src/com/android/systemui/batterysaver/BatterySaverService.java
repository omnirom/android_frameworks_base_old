...
public class BatterySaverService extends Service implements BluetoothConnectionChangeCallback,
    ...
    public void onBluetoothConnectionChange(boolean on, boolean connected) {
        if (!mBatterySaverEnabled || !mSmartBluetoothEnabled) {
            ...
            if (mWasBluetoothEnabled) mWasBluetoothEnabled = false;
            if (mIsBluetoothEnabledByUser) mIsBluetoothEnabledByUser = false;
            return;
        }
        ...
        if (mIsBluetoothEnabledByUser != on) {
    ...
    public void onLocationSettingsChanged(boolean locationEnabled, int locationMode) {
        if (!mBatterySaverEnabled || !mSmartLocationEnabled) {
            // return default value
            if (mWasLocationEnabled) mWasLocationEnabled = false;
            if (mIsLocationEnabledByUser) mIsLocationEnabledByUser = false;
            return;
        }
        // detect user interacting while power saving running
        if (mIsLocationEnabledByUser != locationEnabled) {
    ...
    public void onMobileDataSignalChanged(
        ...
        if (!mBatterySaverEnabled || !mSmartDataEnabled) {
            // return default value
            if (mWasMobileDataEnabled) mWasMobileDataEnabled = false;
            if (mIsMobileDataEnabledByUser) mIsMobileDataEnabledByUser = false;
            return;
        }
        // detect user interacting while power saving running
        if (mIsMobileDataEnabledByUser != isMobileDataEnabled()) {
    ...
    public void onWifiSignalChanged(boolean enabled, int wifiSignalIconId,
        ...
        if (!mBatterySaverEnabled) {
            // return default value
            if (mWasWifiEnabled) mWasWifiEnabled = false;
            if (mIsWifiEnabledByUser) mIsWifiEnabledByUser = false;
            return;
        }
        if (mSmartWifiEnabled) {
            // detect user interacting while power saving running
            if (mIsWifiEnabledByUser != isWifiEnabled()) {
    ...
    private void restoreAllState() {
        ...
        if (deviceSupportsBluetooth() && mSmartBluetoothEnabled) {
            if (mWasBluetoothEnabled || mIsBluetoothDisabledByService) {
        ...
        if (deviceSupportsGps() && mSmartLocationEnabled) {
            if (mWasLocationEnabled || mIsLocationDisabledByService) {
        ...
        if (!mUsbTethered && !isWifiApEnabled()) {
            if (deviceSupportsMobileData() && mSmartDataEnabled) {
                if (mWasMobileDataEnabled || mIsMobileDataDisabledByService) {
            ...
            if (mSmartWifiEnabled) {
                if (mWasWifiEnabled || mIsWifiDisabledByService) {
        ...
        public void run() {
            ...
            if (mCurrentState == State.POWER_SAVING) {
                if ((mWasMobileDataEnabled || mIsMobileDataEnabledByUser)
                    && mSmartDataEnabled) {
                    mCM.setMobileDataEnabled(false);
                    mIsMobileDataDisabledByService = mIsMobileDataEnabledByUser;
                }
        ...
        public void run() {
            ...
            if (mCurrentState == State.POWER_SAVING) {
                if ((mWasBluetoothEnabled || mIsBluetoothEnabledByUser)
                    ...
                    mBluetoothAdapter.disable();
                    mIsBluetoothDisabledByService = mIsBluetoothEnabledByUser;
                }
            } else if (mCurrentState == State.NORMAL) {
                if ((mWasBluetoothEnabled || mIsBluetoothDisabledByService)
                    && mSmartBluetoothEnabled) {
        ...
        public void run() {
            ...
            if (mCurrentState == State.POWER_SAVING) {
                if ((mWasLocationEnabled || mIsLocationEnabledByUser)
                    ...
                    mLocationController.setLocationEnabled(false);
                    mIsLocationDisabledByService = mIsLocationEnabledByUser;
                }
            } else if (mCurrentState == State.NORMAL) {
                if ((mWasLocationEnabled || mIsLocationDisabledByService)
                    && mSmartLocationEnabled) {
        ...
        public void run() {
            ...
            if (mCurrentState == State.POWER_SAVING) {
                if ((mWasWifiEnabled || mIsWifiEnabledByUser) && mSmartWifiEnabled
                     && !isWifiConnected()) {
                    updateWifiState(false);
                    mIsWifiDisabledByService = mIsWifiEnabledByUser;
                }
            } else if (mCurrentState == State.NORMAL) {
                if ((mWasWifiEnabled || mIsWifiDisabledByService) && mSmartWifiEnabled
                     && !isWifiConnected()) {
                    updateWifiState(true);
