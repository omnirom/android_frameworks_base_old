...
constexpr const char* kOmniRomPath = "/system/framework/omnirom-res.apk";
...
idmap2::Result<Idmap2Service::TargetResourceContainerPtr> Idmap2Service::GetTargetContainer(
  ...
  if (target_path == kOmniRomPath) {
    if (omnirom_apk_cache_ == nullptr) {
      // Initialize the lineage APK cache.
      auto target = TargetResourceContainer::FromPath(target_path);
      if (!target) {
        return target.GetError();
      }
      omnirom_apk_cache_ = std::move(*target);
    }
    return {omnirom_apk_cache_.get()};
  }
