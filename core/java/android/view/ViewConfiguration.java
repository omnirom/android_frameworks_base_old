...
public class ViewConfiguration {
    ...
    private ViewConfiguration(Context context) {
        ...
        int scrollFriction = Settings.System.getFloatForUser(resolver,
                          Settings.System.CUSTOM_SCROLL_FRICTION, DEFAULT_SCROLL_FRICTION, UserHandle.USER_CURRENT_OR_SELF);
        if (scrollFriction == 0 || scrollFriction == null) {
            SCROLL_FRICTION = DEFAULT_SCROLL_FRICTION;
        } else {
            SCROLL_FRICTION = scrollFriction;
        }
        int maximumFlingVelocity = Settings.System.getIntForUser(resolver,
                          Settings.System.CUSTOM_FLING_VELOCITY, DEFAULT_MAXIMUM_FLING_VELOCITY, UserHandle.USER_CURRENT_OR_SELF);
        if (maximumFlingVelocity == 0 || maximumFlingVelocity == null) {
            MAXIMUM_FLING_VELOCITY = DEFAULT_MAXIMUM_FLING_VELOCITY;
        } else {
            MAXIMUM_FLING_VELOCITY = maximumFlingVelocity;
        }
        int overScrollDistance = Settings.System.getIntForUser(resolver,
                          Settings.System.CUSTOM_OVERSCROLL_DISTANCE, DEFAULT_OVERSCROLL_DISTANCE, UserHandle.USER_CURRENT_OR_SELF);
        if (overScrollDistance > 100 || overScrollDistance == null) {
            OVERSCROLL_DISTANCE = DEFAULT_OVERSCROLL_DISTANCE;
        } else {
            OVERSCROLL_DISTANCE = overScrollDistance;
        }
        int overFlingDistance = Settings.System.getIntForUser(resolver,
                          Settings.System.CUSTOM_OVERFLING_DISTANCE, DEFAULT_OVERFLING_DISTANCE, UserHandle.USER_CURRENT_OR_SELF);
        if (overFlingDistance > 100 || overFlingDistance == 0 || overFlingDistance == null) {
