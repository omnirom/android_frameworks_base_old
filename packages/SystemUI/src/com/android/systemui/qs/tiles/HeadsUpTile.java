...
public class HeadsUpTile extends QSTile<QSTile.BooleanState> {
    private final AnimationIcon mHeadsupOnToOff
            = new AnimationIcon(R.drawable.ic_qs_headsup_to_off,
            R.drawable.headsup_on);
    private final AnimationIcon mHeadsupOffToOn
            = new AnimationIcon(R.drawable.ic_qs_headsup_to_on,
            R.drawable.headsup_off);
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        if (getUserHeadsUpState()) {
            state.icon = mHeadsupOnToOff;
            state.contentDescription =  mContext.getString(
                    R.string.quick_settings_heads_up_on);
        } else {
            state.icon = mHeadsupOffToOn;
            state.contentDescription =  mContext.getString(
    ...
    protected String composeChangeAnnouncement() {
        if (mState.value) {
            return mContext.getString(
                    R.string.quick_settings_heads_up_changed_on);
        } else {
            return mContext.getString(
                    R.string.quick_settings_heads_up_changed_off);
        }
    }
