...
public class BatteryMeterHorizontalView extends AbstractBatteryView {
    ...
    public void draw(Canvas c) {
        ...
        final int insetTop = (height - mBarHeight) / 2 + 4;
        final int insetBottom = (height - mBarHeight) / 2 - 4;
        ...
        final float levelTop = drawFrac == 1f ? mButtonFrame.right
                : (mFrame.left + (mFrame.width() * (1f - drawFrac)));
        ...
        if (tracker.plugged) {
            ...
            final float bt = mFrame.top + insetTop + mFrame.height() / 6f;
            final float br = mFrame.right - mFrame.width() / 7f;
            final float bb = mFrame.bottom - insetBottom - mFrame.height() / 10f;
