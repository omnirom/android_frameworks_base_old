...
public class BatterySaverService extends Service {
    ...
    public void onCreate() {
        ...
        Log.d(TAG, "started BatterySaver");
        Toast.makeText(mContext, "Battery Saver : Enabled", Toast.LENGTH_SHORT).show();
    }
    ...
    private void switchToState(State newState, boolean force, boolean withWakeLock) {
        if (mCurrentState == newState && !force) {
            return;
        } else if (!mBatterySaverEnabled || isOnCall() || mUsbTethered || isWifiApEnabled()) {
            return;
        }
    ...
    private void set2G3G(int network) {
        Toast.makeText(mContext, "Battery Saver : Change network to " + getNetworkType(network, mContext.getResources()), Toast.LENGTH_SHORT).show();
        switch(network) {
            case Phone.NT_MODE_GLOBAL:
    ...
    private class NetworkModeChanger implements Runnable {
        ...
        public NetworkModeChanger(Context context) {
            ...
            mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "SystemUI:BatterySaver");
        }
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.d(TAG, "stopped BatterySaver");
        Toast.makeText(mContext, "Battery Saver : Disabled", Toast.LENGTH_SHORT).show();
