...
public class ActiveDisplayView extends FrameLayout
    ...
    private long mShakeLongThreshold = 3000L;
    ...
    public synchronized void onShake() {
        if ((mDisableShakeQuite && inQuietHours()) || isOnCall()) {
            return;
        }

        if (!isScreenOn() && !mIsActive && mEnableShake) {
        ...
        } else if (isScreenOn() && mIsActive && mWakedByShakeMode && mEnableShake) {
            if ((System.currentTimeMillis() >= (mShakeTime + mShakeLongThreshold)) && (mShakeTime != 0)) {
