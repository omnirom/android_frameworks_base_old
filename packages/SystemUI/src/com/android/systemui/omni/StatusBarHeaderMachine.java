...
public class StatusBarHeaderMachine {
    ...
    private Context mContext;
    private List<IStatusBarHeaderMachineProvider> mProviders = new ArrayList<IStatusBarHeaderMachineProvider>();
    ...
    public Drawable getCurrent() {
        ...
        if (mProviders.size() > 0) {
            Iterator<IStatusBarHeaderMachineProvider> nextProvider = mProviders
                    .iterator();
            while (nextProvider.hasNext()) {
                IStatusBarHeaderMachineProvider provider = nextProvider.next();
    ...
    public void addProvider(IStatusBarHeaderMachineProvider provider) {
        if (!mProviders.contains(provider)) {
            mProviders.add(provider);
        }
    }

    public void removeProvider(IStatusBarHeaderMachineProvider provider) {
    ...
    private void doUpdateStatusHeaderObservers() {
        ...
        while (nextObserver.hasNext()) {
            ...
            try {
                observer.updateHeader(getCurrent());
            } catch (Exception e) {
                // just in case
            }
        }
