...
public class TileLayout extends ViewGroup implements QSTileLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int columns = getMaxVisibleColumns(mSettingsColumns);
        if (columns != 0) {
    ...
    public boolean updateMaxRows(int allowedHeight, int tilesCount) {
        ...
        Log.d(TAG, "availableHeight = " + availableHeight + " mCellHeight = " + mCellHeight + " mRows = " +mRows + " mMaxAllowedRows = " + mMaxAllowedRows);
        if (mRows < mMinRows) {
            mRows = mMinRows;
    ...
    protected int getMaxVisibleColumns(int columns) {
        int maxColumns = 0;

        final int cellWidth = mContext.getResources().getDimensionPixelSize(R.dimen.qs_quick_tile_size);
