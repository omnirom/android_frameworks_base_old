...
public class GlobalActionsDialog implements DialogInterface.OnDismissListener,
    ...
    private boolean advancedRebootEnabled(Context context) {
        ...
        return advancedRebootEnabled && mRebootMenuActions.length > 1;
    }
    ...
    private class ScreenshotAction extends SinglePressAction implements LongPressAction {
        ...
        public void onPress() {
            ...
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    mScreenshotHelper.takeScreenshot(TAKE_SCREENSHOT_FULLSCREEN, true, true, mHandler, null);
        ...
        public boolean onLongPress() {
            mHandler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    mScreenshotHelper.takeScreenshot(TAKE_SCREENSHOT_SELECTED_REGION, true, true, mHandler, null);
                    MetricsLogger.action(mContext,
                            MetricsEvent.ACTION_SCREENSHOT_POWER_MENU);
                }
            }, 500);
