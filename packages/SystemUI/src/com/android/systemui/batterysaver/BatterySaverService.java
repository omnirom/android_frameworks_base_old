...
public class BatterySaverService extends Service implements BluetoothConnectionChangeCallback,
    ...
    private long mLastUpdateIntervalTime = 0;
    ...
    private void autoSwitchAfterScreenTurnOff() {
        ...
        if (mIsScreenOff && (mUserCheckIntervalTime != 0)
            && (mLastUpdateIntervalTime != 0) && (mLastCheckIntervalTime != 0)) {
              if (System.currentTimeMillis() > (mLastUpdateIntervalTime + mUserCheckIntervalTime)) {
                  if (!mPowerConnected && shouldSwitch() && !mWifiEvent) {
    ...
    private void autoCheckNetworkMode() {
        if (!mSignalEvent && mSmartNoSignalEnabled && mMobileDataModeChanger.isSupported()) {
    ...
    private void restoreAllState() {
        ...
        if (mobiledata && network && bluetooth && location && wifi && brightness && cpufreq && syncs) {
            showToast(7);
        } else if (!mobiledata && !network && !bluetooth && !location && !wifi && !brightness && !cpufreq && !syncs) {
            showToast(8);
        }
    }
    ...
    private void switchToState(State newState, boolean checkInterval) {
        switchToState(newState, false, checkInterval);
    }

    private void switchToState(State newState, boolean force, boolean checkInterval) {
        ...
        switch (newState) {
              case NORMAL:
                   ...
                   if (checkInterval) {
                       mLastUpdateIntervalTime = System.currentTimeMillis();
                   }
                   normalize = true;
                   break;
              case POWER_SAVING:
        ...
        if ((!mWifiEvent && !checkInterval) || (force && !checkInterval)) {
