...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        mPowerKeyHandled = hungUp || mScreenshotChordVolumeDownKeyTriggered
                || mScreenrecordChordVolumeDownKeyTriggered
    ...
    private void interceptPowerKeyUp(KeyEvent event, boolean interactive, boolean canceled) {
        ...
        mScreenrecordChordPowerKeyTriggered = false;
    ...
    private void interceptScreenrecordChord() {
        if (mScreenrecordChordEnabled
                && mScreenrecordChordVolumeUpKeyTriggered && mScreenrecordChordPowerKeyTriggered
                && !mA11yShortcutChordVolumeDownKeyTriggered) {
            final long now = SystemClock.uptimeMillis();
            if (now <= mScreenrecordChordVolumeUpKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS
                    && now <= mScreenrecordChordPowerKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS) {
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (mScreenrecordChordEnabled && (flags & KeyEvent.FLAG_FALLBACK) == 0) {
            if (mScreenrecordChordVolumeUpKeyTriggered && !mScreenrecordChordPowerKeyTriggered) {
                final long now = SystemClock.uptimeMillis();
                final long timeoutTime = mScreenrecordChordVolumeDownKeyTime + SCREENSHOT_CHORD_DEBOUNCE_DELAY_MILLIS;
                if (now < timeoutTime) {
                    return timeoutTime - now;
                }
            }
