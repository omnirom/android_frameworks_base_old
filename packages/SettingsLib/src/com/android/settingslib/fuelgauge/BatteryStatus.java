...
public class BatteryStatus {
    ...
    public final int getChargingSpeed(Context context) {
        ...
        final boolean dashCharging = context.getResources().getBoolean(
                R.bool.config_dashCharging);
        if (dashCharging && (maxChargingWattage == 250000)) {
            maxChargingWattage = maxChargingWattage * 100;
        }
        int dash = maxChargingWattage > dashThreshold ? CHARGING_DASH :
                                CHARGING_FAST;
        return maxChargingWattage <= 0 ? CHARGING_UNKNOWN :
                maxChargingWattage < slowThreshold ? CHARGING_SLOWLY :
                        maxChargingWattage > fastThreshold ? dash :
                                CHARGING_REGULAR;
    }
