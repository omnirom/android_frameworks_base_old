...
public class NetworkTraffic extends TextView {
    ...
    private Handler mTrafficHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (shouldHide(rxData, txData, timeDelta)) {
                ...
                if (isSet(mState, MASK_UP)) {
                    output = "U:" + formatOutput(timeDelta, txData);
                    output += symbol;
                }
                ...
                if (isSet(mState, MASK_DOWN)) {
                    output += "D:" + formatOutput(timeDelta, rxData);
                    output += symbol;
                }
        ...
        private String formatOutput(long timeDelta, long data) {
            ...
            String output = "";
            if (speed < KB) {
                output = decimalFormat.format(speed);
            } else if (speed < MB) {
                output = decimalFormat.format(speed / (float)KB) + 'k';
            } else if (speed < GB) {
                output = decimalFormat.format(speed / (float)MB) + 'M';
            } else {
                output = decimalFormat.format(speed / (float)GB) + 'G';
            }
            return String.format("%5s", output);
        }
